 ...
 */
public class JenkinsHTMLReport implements IReporter 
{
  private static final Logger L = Logger.getLogger( JenkinsHTMLReport.class );
  ...
  private static String outFilename = "jenkins-report.html";
    ...
    if ( tests.getAllResults().size() > 0 ) {
      ...
      for ( ITestNGMethod method : getMethodSet(tests, suite) ) {
        ...
        buff.append("<td><a href=\"#m" + m_methodIndex + "\">"
            ...
            + "<td>" + parseUnixTimeToTimeOfDay(start) + "</td>"
            + "<td class=\"numi\">" + millisecondsToSeconds(end - start).toPlainString() + "</td>" //TODO need to enhance so the time of each method is shown separately
            + "</tr>");
      }
      if (mq > 0) {
        cq += 1;
  ...
  private void startResultSummaryTable(String style) {
    ...
    m_out.println("<tr><th>Class</th>"
            + "<th>Method</th><th># of<br/>Scenarios</th><th>Start</th><th>Time<br/>(seconds)</th></tr>");
    m_row = 0;
  }
  ...
  protected void startHtml(PrintWriter out) {
    ...
    out.println("</head>");
    out.println("<body>");
	out.println( "<h2>Report creation date: " + parseUnixTimeToDate(new Date().getTime()) + "</h2><br/>" );
  }
...
//      return r;
  ...
  protected BigDecimal millisecondsToSeconds( long ms ) {
